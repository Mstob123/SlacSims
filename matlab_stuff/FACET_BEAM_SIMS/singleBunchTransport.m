function [data,BEAMLINE] = singleBunchTransport(simparams, endEl, initialBeam, BEAMLINE)

% add paths
%for change linacphases func and other helper functions
...
if ~exist('initialBeam')
      b =  load('/home/mstobbe/matlab_scripts/facet2-lattice-master/Lucretia/beams/FACET2e_op.mat');
      initialBeam = b.Beam;
end
...
[~,data(length(ele)+1).beam]=TrackThru(ele(end),endEl,beam,1,1);
        ...
        [~,~,~,data(length(ele)+1).I,data(length(ele)+1).Eprof]=MakeBeamLPS(beam,zbins,ebins,0);  
