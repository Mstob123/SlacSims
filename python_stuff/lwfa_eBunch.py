...
zmax = 90.e-6    # Right end of the simulation box (meters)
...
p_zmax = 5.e-3 # Position of the end of the plasma (meters)
p_rmax = 100.e-6  # Maximal radial position of the plasma (meters)
n_e = 1.e17*1.e6 # Density (electrons.meters^-3)
...
def dens_func( z, r ):
    ...
    n = np.where(z>ramp_end, 0., n)
    return(n)

# The interaction length of the simulation (meters)
L_interact = 5.e-3 # increase to simulate longer distance!
...
if __name__ == '__main__':
    ...
    bunch = add_particle_bunch_file(sim, -chargeMag, mass, 'fbFull.txt', macroN*weight)
    ...
    sim.diags = [ FieldDiagnostic( diag_period, sim.fld, comm=sim.comm ),
                  ParticleDiagnostic( diag_period, {"electrons_plasma" : elec, "electrons_beam" : bunch},
